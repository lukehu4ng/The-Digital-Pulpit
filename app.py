                ...
                try:
                    ...
                    with st.spinner("Processing..."):
                        ...
                        try:
                            ...
                            # download at a low bitrate so the *whole* file fits under 24 MB
                            ydl_opts = {
                            ...
                            # Set the chosen model and check if we need to split
                            is_gpt4o_model = transcription_model in ["gpt-4o-transcribe", "gpt-4o-mini-transcribe"]
                            
                            # --- Audio splitting for long videos (especially needed for GPT-4o models) -------------
                            if is_gpt4o_model and video_duration and video_duration > MAX_SEC:
                                download_progress.progress(70, text="Splitting audio into segments...")
                                st.warning(
                                    f"Video is {video_duration/60:.1f} min (> {MAX_SEC/60:.1f} min). "
                                    f"Splitting into chunks for {transcription_model}."
                                )
                                audio = AudioSegment.from_file(audio_path)
                                audio_duration_ms = len(audio)
                                step_ms = MAX_SEC * 1000  # pydub uses milliseconds
                                audio_segments_to_process = []
                                
                                # Calculate how many segments we need
                                num_segments = (audio_duration_ms + step_ms - 1) // step_ms  # Ceiling division
                                
                                for i in range(num_segments):
                                    # Update progress based on segment progress
                                    split_progress = 70 + int(30 * ((i + 1) / num_segments))
                                    download_progress.progress(split_progress, text=f"Processing segment {i+1}/{num_segments}...")
                                    start_ms = i * step_ms
                                    end_ms = min(start_ms + step_ms, audio_duration_ms)
                                    
                                    # Make sure we don't go over the audio length
                                    if start_ms >= audio_duration_ms:
                                        break
                                        
                                    # Extract the segment
                                    segment = audio[start_ms:end_ms]
                                    seg_path = os.path.join(temp_dir, f"{video_id}_part{i+1}.mp3")
                                    segment.export(seg_path, format="mp3")
                                    
                                    # Verify exported segment length in seconds
                                    segment_audio = AudioSegment.from_file(seg_path)
                                    segment_duration_sec = len(segment_audio) / 1000
                                    
                                    if segment_duration_sec > MAX_SEC and is_gpt4o_model:
                                        st.warning(f"Segment {i+1} duration ({segment_duration_sec:.1f}s) exceeds limit. "
                                                  f"Will fall back to Whisper model for this segment if needed.")
                                    
                                    audio_segments_to_process.append(seg_path)
                            else:
                                # For Whisper-1 model or short videos, no need to split
                                download_progress.progress(80, text="No splitting needed, preparing audio...")
                                audio_segments_to_process = [audio_path]
                                
                                if is_gpt4o_model and (not video_duration or video_duration > MAX_SEC):
                                    st.warning(f"Unable to determine audio duration or it exceeds limits. May need to fall back to Whisper model if {transcription_model} fails.")
